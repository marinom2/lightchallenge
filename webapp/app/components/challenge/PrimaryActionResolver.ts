...
export function resolvePrimaryAction(ctx: {
  ...
  if (shouldShowClaims) {
    return {
      ...
      desc: `${claimablesCount} reward${claimablesCount === 1 ? "" : "s"} ready`,
  ...
  // Page passes needsSettlement; use it to avoid showing “Finalize” too early.
  if (
    isAdmin &&
    (String(effectiveStatus) === "Approved" || String(effectiveStatus) === "Paused") &&
    needsSettlement
  ) {
