...
export default function SteamLinkButton({ subject, className="", onLinked, loadingExternal=false, currentBinding }: Props) {
  ...
  return (
    <div className={["rounded-2xl border border-[color:var(--border)] bg-[color:var(--surface-1)] p-3", className].join(" ")}>
      <div className="flex items-center justify-between gap-3">
        <div className="text-sm">
          {loadingExternal ? <span className="text-[color:var(--text-muted)]">Loading Steam status…</span> : linkedText}
        ...
        <div className="flex items-center gap-2">
          <button className="rounded-xl border border-[color:var(--border)] px-3 py-1.5 hover:bg-[color:var(--surface-2)]" onClick={startLink}>
            {currentBinding ? "Relink" : "Link Steam"}
          </button>
          {currentBinding && (
            <a className="text-xs text-[color:var(--text-muted)] underline hover:no-underline"
