...
Finds challenges in their **proof submission window** and performs final reconciliation
evidence collection. See the root `OPERATIONS.md` for full details.
...
Runs during **active challenges** to keep real-time progress updated. Syncs from ALL
API-based providers every 15 minutes and performs a final reconciliation fetch during the proof window.
...
- `opendota` — free public API (gaming: Dota 2 matches)
- `riot` — API key required (gaming: League of Legends matches)
- `faceit` — API key required (gaming: CS2 matches)

**Upload-only providers (NOT synced — no server-side API):**
- `apple` — data pushed from iOS AutoProofService (HealthKit) every ~15 min during active period
...
npx tsx offchain/workers/progressSyncWorker.ts      # active-period progress sync (every 15min)
npx tsx offchain/workers/evidenceCollector.ts       # proof-window final reconciliation
...
# OR start all workers via PM2 (production):
...
> which must be running before `challengeWorker`. The indexers and `progressSyncWorker` are independent.
