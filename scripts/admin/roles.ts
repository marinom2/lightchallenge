...
function needAdminSigner() {
  const pk = process.env.ADMIN_PRIVATE_KEY;
  if (!pk) throw new Error("ADMIN_PRIVATE_KEY is missing in env");
  if (!pk.startsWith("0x") || pk.length !== 66) {
    throw new Error("ADMIN_PRIVATE_KEY must be 32-byte hex, like 0x + 64 hex chars");
  }
...
async function getTreasuryInstance(addr?: string) {
  ...
  if (!treasury) {
    throw new Error(
      "Treasury address not provided. Pass TREASURY env or ensure webapp/public/deployments/lightchain.json has { contracts: { Treasury: <addr> } }."
    );
  }

  // IMPORTANT: Use ADMIN_PRIVATE_KEY signer (not default hardhat signer)
  const admin = needAdminSigner();
  return ethers.getContractAt("Treasury", treasury, admin);
}
...
async function main() {
  ...
  const signerAddr = await (tr.runner as any)!.getAddress?.();
  const adminOfRole = await tr.getRoleAdmin(roleId);
  const signerHasAdmin = await tr.hasRole(adminOfRole, signerAddr);
  ...
  console.log(`Signer has admin-of-role? ${signerHasAdmin ? "YES" : "NO"}`);
  ...
  switch (ACTION) {
    case "list": {
      ...
      if (!holders.length) console.log("No current holders detected from event history.");
      else for (const h of holders) console.log(`- ${h}`);
      break;
    }

    case "check": {
      if (!targetArg) throw new Error("Missing TARGET (address)");
    ...
    case "renounce": {
      // renounce uses the same admin signer; only works if signer == target
      const admin = needAdminSigner();
      const who = ethers.getAddress(targetArg || (await admin.getAddress()));
      if (who.toLowerCase() !== (await admin.getAddress()).toLowerCase()) {
        console.warn("renounceRole can only be called by the account itself; ignoring TARGET");
      }
      console.log(`Renouncing ${prettyRoleName(roleArg || undefined, roleId)} for ${await admin.getAddress()}...`);
      const tx = await tr.renounceRole(roleId, await admin.getAddress());
      await tx.wait();
    ...
    default: {
      console.log(
        [
          "",
          "Usage (ENV recommended, avoids HH305):",
          "  ACTION=list  ROLE=operator TREASURY=0x... npx hardhat run scripts/admin/roles.ts --network lightchain",
          "  ACTION=check ROLE=operator TREASURY=0x... TARGET=0x... npx hardhat run scripts/admin/roles.ts --network lightchain",
          "  ACTION=grant ROLE=operator TREASURY=0x... TARGET=0x... npx hardhat run scripts/admin/roles.ts --network lightchain",
          "  ACTION=revoke ROLE=operator TREASURY=0x... TARGET=0x... npx hardhat run scripts/admin/roles.ts --network lightchain",
        ].join("\n")
      );
    }
  }
